...
namespace gef
{
    ...
    void PNGLoader::Load(const char* filename, const Platform& platform, ImageData& image_data)
    {
        ...
        if(!png_file->Open(filename)){
            gef::DebugOut("Could not open file: %s", filename);
            return;
        }
        
        Int32 file_size;
        if(!png_file->GetSize(file_size)) {
            gef::DebugOut("Could not get file size: %s", filename);
            return;
        }
        std::vector<UInt8> buffer{ (size_t)file_size };
        Int32 bytes_read;

        if(!png_file->Read(buffer.data(), file_size, bytes_read)){
            gef::DebugOut("Could not read file data: %s", filename);
            return;
        }
        if(!png_file->Close()){
            gef::DebugOut("Could not close file: %s", filename);
            return;
        }
        png_structp png_ptr;
        png_infop info_ptr;
        ...
        if(png_ptr == NULL)
        {
            gef::DebugOut("Could not create png struct: %s", filename);
            return;
        }
        ...
        case PNG_COLOR_TYPE_RGB:
            ParseRGB(image_data.image(), png_ptr, info_ptr, width, height);
            break;
    ...
    void PNGLoader::ParseRGBA(UInt8* out_image_buffer, png_structp the_png_ptr, const png_infop the_info_ptr, UInt32 width, UInt32 height)
    {
